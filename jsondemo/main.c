...
int main() {
    ...
    cJSON_AddNumberToObject(item, "age", 8);
    cJSON_AddNumberToObject(item, "gender", 0);
    cJSON_AddStringToObject(item, "address", "America");
    cJSON_AddItemToArray(json,item);

    printf("%s\n",cJSON_Print(json));
    file=fopen("./data/user_re.json","wb");
    char *target=cJSON_Print(json);
    fwrite(target, sizeof(char),strlen(target),file);
    fclose(file);
